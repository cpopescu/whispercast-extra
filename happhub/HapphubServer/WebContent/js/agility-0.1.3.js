    ...
    view: {
      ...
      bindings: function(){
        ...
        $rootNode.add($childNodes).each(function(){
          ...
          // <ul>, <ol>s: 1-way binding
          else if ($node.is('ul, ol')) {
            if (bindData.key) {
              self.bind('_change:'+bindData.key, function(){
                if (self.model.get(bindData.key)) {
                  $node.html('');
                  $.each(self.model.get(bindData.key), function(i,e) {
                    if ($node.data('markup')) {
                      var $inner = $($node.data('markup'));
                      if ($node.data('member')) {
                        $inner.text(e[$node.data('member')]);
                      } else {
                        $inner.text(e);
                      }
                      $node.append($('<li></li>').append($inner));
                    } else {
                      if ($node.data('member')) {
                        $node.append($('<li></li>').text(e[$node.data('member')]));
                      } else {
                        $node.append($('<li></li>').text(e));
                      }
                    }
                  });
                } else {
    ...
    this.save = function(){
      ...
      this._data.persist.adapter.call(this, {
        ...
        success: function(data, textStatus, jqXHR){
          // @@@ Happhub: Reload the entire model from the result
          /*
          if (data[id]) {
            // id in body
